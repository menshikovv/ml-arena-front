...
export default function Leaderboard() {
  ...
  return (
    <PageFrame>
      ...
      <Reveal className="mt-6" delay={0.04}><div className="grid gap-px border border-border bg-border sm:grid-cols-4">{[[CalendarClock, "Сезон", archived ? "Beta Season" : "Founder Season", archived ? "архив" : "до 31 декабря"], [Users, "Участников", sourceRows.length || "—", "с рейтинговой активностью"], [Trophy, "Событий", 10, "завершено"], [Clock3, "Обновление", "сегодня", "актуальные результаты"]].map(([Icon, label, value, detail]) => <div key={label} className="bg-card p-4"><div className="flex items-center gap-2 text-xs text-muted-foreground"><Icon size={14} className="text-primary" />{label}</div><p className="mt-3 font-heading text-xl font-extrabold">{value}</p><p className="mt-1 text-[10px] text-muted-foreground">{detail}</p></div>)}</div></Reveal>
...
export function RatingMethodology() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1380px] px-4 py-6 md:px-6 lg:px-8 lg:py-10">
      ...
      <header className="mt-7 border-b border-border pb-7"><h1 className="font-heading text-3xl font-extrabold leading-tight sm:text-4xl lg:text-5xl">Как считается рейтинг</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground sm:text-base">{method.description || "Рейтинг начинается заново в каждом сезоне, а подтверждённые результаты прошлых сезонов остаются в ML-паспорте и архиве."}</p><div className="mt-5"><span className="border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground">{method.competition_weight_percent ?? 70}% соревнования · {method.duel_weight_percent ?? 30}% дуэли</span></div></header>
