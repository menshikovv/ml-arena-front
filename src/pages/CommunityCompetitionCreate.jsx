      ...
      <div className="mt-7 grid gap-7 lg:grid-cols-[250px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 border border-border bg-card">
          <div className="border-b border-border px-5 py-5 sm:px-7">
            <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center bg-primary/10 text-primary">{React.createElement(STEPS[step].icon, { size: 19 })}</span><h2 className="font-heading text-xl font-extrabold">{STEPS[step].label}</h2></div>
          </div>
  ...
  return (
    <div>
      <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
        <div className="border border-border bg-secondary/25 p-5"><h3 className="font-heading text-2xl font-extrabold">{form.title || "Название соревнования"}</h3><div className="mt-4 flex flex-wrap gap-2"><span className="bg-violet-500/10 px-2 py-1 text-xs font-bold text-violet-600 dark:text-violet-400">Сообщество</span><span className="bg-secondary px-2 py-1 text-xs font-bold">{accessLabel}</span><span className="bg-secondary px-2 py-1 text-xs font-bold">{TASK_TYPE_LABELS[form.direction] || form.customDirection}</span></div><p className="mt-3 text-sm leading-6 text-muted-foreground">{form.description || "Короткое описание появится здесь."}</p><div className="mt-6 grid grid-cols-2 gap-3 border-t border-border pt-5 text-xs"><div><span className="text-muted-foreground">Метрика</span><p className="mt-1 font-bold">{form.metric || "—"}</p></div><div><span className="text-muted-foreground">Попытки</span><p className="mt-1 font-bold">{form.attempts} в сутки</p></div><div><span className="text-muted-foreground">Рейтинг</span><p className="mt-1 font-bold">Не влияет</p></div><div><span className="text-muted-foreground">Призы</span><p className="mt-1 font-bold">Без денежных призов</p></div></div></div>
