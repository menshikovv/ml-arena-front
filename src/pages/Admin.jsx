...
function CompetitionEditorDialog({ competitionId, creating, onClose, onSaved }) {
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !mutation.isPending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[94vh] w-[calc(100%-2rem)] max-w-5xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between border-b border-border p-5 md:px-7"><div><Dialog.Title className="font-heading text-2xl font-extrabold">{creating ? "Новое соревнование" : "Редактирование соревнования"}</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Содержимое, ресурсы, сроки и правила события.</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-7">{detail.isLoading && !creating ? <LoadingRows /> : detail.error ? <ErrorState error={detail.error} /> : <div className="space-y-8">
    ...
    <section><h3 className="font-heading text-lg font-bold">Ресурсы и проверка</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><AdminField label="ID организации"><Input value={form.organization_id} onChange={(event) => update("organization_id", event.target.value)} disabled={!creating} className="rounded-none" /></AdminField><AdminField label="ID версии задачи"><Input value={form.task_version_id} onChange={(event) => update("task_version_id", event.target.value)} className="rounded-none" /></AdminField><AdminField label="ID версии датасета"><Input value={form.dataset_version_id} onChange={(event) => update("dataset_version_id", event.target.value)} className="rounded-none" /></AdminField><AdminField label="ID версии метрики"><Input value={form.metric_version_id} onChange={(event) => update("metric_version_id", event.target.value)} className="rounded-none" /></AdminField>{creating && <><AdminSelect label="Тип задачи" value={form.task_type} onChange={(value) => update("task_type", value)} options={TASK_TYPE_OPTIONS} /><AdminField label="Код метрики"><Input value={form.metric_code} onChange={(event) => update("metric_code", event.target.value)} className="rounded-none" /></AdminField><AdminSelect label="Доступ" value={form.access} onChange={(value) => update("access", value)} options={[["open", "Открытое"], ["invite_only", "По приглашению"], ["partner", "Партнёрское"], ["premium", "Premium"]]} /></>}</div></section>
...
function ResourceFields({ type, mode, form, update }) {
...
function ResourceEditorDialog({ editor, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(saveError instanceof SyntaxError ? "Проверьте корректность JSON-полей." : saveError.message);
    } finally {
      setPending(false);
    }
  };
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !pending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[92vh] w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between gap-5 border-b border-border p-5 md:px-6"><div><Dialog.Title className="font-heading text-2xl font-extrabold">{title}</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Поля и переходы повторно проверяются сервером.</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-6"><ResourceFields type={editor.type} mode={editor.mode} form={form} update={update} />{error && <div className="mt-5 border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}</div><div className="flex justify-end gap-2 border-t border-border bg-card/95 p-4"><Button variant="outline" onClick={onClose} disabled={pending}>Отмена</Button><Button onClick={save} disabled={pending}>{pending ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Сохранить</Button></div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
...
function DatasetFilesDialog({ datasetId, permissions, onClose, onSaved }) {
  ...
  const attach = async () => {
    ...
    try {
      ...
      await api.admin.attachDatasetFile(datasetId, version.id, { upload_id: upload.id, kind, visibility: kind === "private_labels" ? "evaluator_only" : visibility, position: files.length });
      setFile(null);
      await query.refetch();
      onSaved();
    } catch (uploadError) {
