...
const STATUS_LABELS = {
  ...
  pending_tests: "Тестируется", tests_failed: "Тесты не пройдены", pending_review: "Ожидает решения",
  changes_requested: "Нужны изменения", approved: "Одобрена", disabled: "Отключена",
};
...
const SECTIONS = [
  ...
  { id: "moderation", label: "Модерация", icon: ShieldAlert, any: ["moderation.read", "metrics.read"] },
...
function apiErrorMessage(error) {
  ...
  if (!validationErrors.length) {
    const messages = {
      TASK_PURPOSE_LOCKED: "Назначение этой версии уже закреплено. Создайте новую версию задачи.",
      TASK_PURPOSE_MISMATCH: "Назначение версии не подходит для этой операции.",
      DATASET_PURPOSE_CONFLICT: "Датасет уже закреплён за другим типом использования. Выберите или создайте совместимую версию датасета.",
      TASK_ALREADY_USED_BY_COMPETITION: "Эта версия задачи уже используется другим соревнованием.",
      PARTICIPANT_BUNDLE_REQUIRED: "Для duel-версии нужен participant_bundle с доступом participant.",
      DATASET_VALIDATION_REQUIRED: "Перед использованием повторно запустите и успешно завершите проверку датасета.",
    };
    return messages[error?.code] || error?.message || "Не удалось сохранить данные";
  }
...
function competitionForm(item = {}) {
  return {
    ...
    prize_type: item.prize_type || (Number(item.cash_prize_amount_rub || item.prize_amount) > 0 ? "cash" : item.prize_description ? "non_cash" : "none"),
    cash_prize_amount_rub: String(item.cash_prize_amount_rub ?? (Number(item.prize_amount || 0) / 100)),
...
function competitionPayload(form, creating, selectedTask, selectedMetric) {
  const common = {
    ...
    prize_description: form.prize_type === "non_cash" ? nullable(form.prize_description.trim()) : null,
...
function CompetitionEditorDialog({ competitionId, creating, onClose, onSaved }) {
  ...
  const taskOptions = listRows(tasksQuery.data).filter((item) => {
    ...
    return version?.id === form.task_version_id
      || (item.status !== "archived" && version?.exposure_status === "private" && version?.purpose === "competition");
  });
  ...
  const submit = () => {
    ...
    const deadline = new Date(form.submission_deadline).getTime();
    const finalResultsAt = form.final_results_at ? new Date(form.final_results_at).getTime() : null;
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !mutation.isPending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[94vh] w-[calc(100%-2rem)] max-w-5xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between border-b border-border p-5 md:px-7"><div><Dialog.Title className="font-heading text-2xl font-extrabold">{creating ? "Новое соревнование" : "Редактирование соревнования"}</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Содержимое, ресурсы, сроки и правила события.</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-7">{detail.isLoading && !creating ? <LoadingRows /> : detail.error ? <ErrorState error={detail.error} /> : <div className="space-y-8">
    ...
    <section className="border-b border-border pb-8"><div className="flex items-center gap-3"><span className="font-mono text-xs font-bold text-primary">04</span><h3 className="font-heading text-lg font-bold">Призы и оформление</h3></div><div className="mt-4 grid gap-4 md:grid-cols-3"><AdminField label="Тип приза"><AdminSelect value={form.prize_type} onChange={(value) => update("prize_type", value)} options={[["none", "Без призов"], ["cash", "Денежный приз"], ["non_cash", "Неденежные призы"]]} /></AdminField>{form.prize_type === "cash" && <AdminField label="Общий фонд, ₽"><Input type="number" min="1" step="1" value={form.cash_prize_amount_rub} onChange={(event) => update("cash_prize_amount_rub", event.target.value)} className="rounded-none" /></AdminField>}<AdminField label="Цвет страницы"><Input type="color" value={form.banner_color} onChange={(event) => update("banner_color", event.target.value)} className="h-10 rounded-none p-1" /></AdminField>{form.prize_type === "non_cash" && <AdminField label="Описание призов" wide><Textarea maxLength={500} value={form.prize_description} onChange={(event) => update("prize_description", event.target.value)} placeholder="Например: техника, стажировка и менторская сессия для победителей" className="min-h-24 rounded-none" /></AdminField>}</div></section>
...
function CompetitionOperationsDialog({ competition, permissions, onClose, onChanged }) {
  ...
  const users = useQuery({ queryKey: ["admin", "competition-operation-users"], queryFn: () => api.admin.users({ status: "active", limit: 50, offset: 0 }), enabled: can(permissions, "leaderboards.recalculate") });
  const datasets = useQuery({ queryKey: ["admin", "competition-operation-datasets"], queryFn: () => api.admin.datasets({ limit: 50, offset: 0 }), enabled: can(permissions, "leaderboards.recalculate") });
  const metrics = useQuery({ queryKey: ["admin", "competition-operation-metrics"], queryFn: () => api.admin.metrics({ limit: 50, offset: 0 }), enabled: can(permissions, "leaderboards.recalculate") });
  const userOptions = listRows(users.data).map((item) => ({ id: item.id, label: displayName(item) }));
  const datasetOptions = listRows(datasets.data).filter((item) => item.status !== "archived" && item.current_version?.id).map((item) => ({ id: item.current_version.id, label: `${item.name} · версия ${item.current_version.version}` }));
  const metricOptions = listRows(metrics.data).filter((item) => item.status === "active" && item.current_version?.id).map((item) => ({ id: item.current_version.id, label: `${item.name || item.code} · версия ${item.current_version.version}` }));
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !mutation.isPending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[94vh] w-[calc(100%-2rem)] max-w-5xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between border-b border-border p-5 md:px-7"><div><Dialog.Title className="font-heading text-2xl font-extrabold">Готовность и рейтинг</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">{displayName(competition)}</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-7"><div className="space-y-8">
    ...
    {can(permissions, "leaderboards.recalculate") && <section><h3 className="font-heading text-lg font-bold">Пересчёт рейтинга</h3><div className="mt-4 grid gap-3 md:grid-cols-[180px_1fr_1fr_auto]"><select value={scope} onChange={(event) => setScope(event.target.value)} className="h-10 border border-border bg-background px-3 text-sm"><option value="competition">Всё соревнование</option><option value="user">Один участник</option></select>{scope === "user" ? <select value={userId} onChange={(event) => setUserId(event.target.value)} className="h-10 border border-border bg-background px-3 text-sm"><option value="">Выберите участника</option>{userOptions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select> : <div />}<Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Причина пересчёта" className="rounded-none" /><Button onClick={() => mutation.mutate({ type: "recalculate" })} disabled={reason.trim().length < 3 || (scope === "user" && !userId)}><RefreshCw size={15} /> Пересчитать</Button></div><div className="mt-4 divide-y divide-border border border-border">{listRows(recalculations.data).slice(0, 6).map((job) => <div key={job.id} className="flex items-center justify-between gap-3 p-3 text-sm"><div><Status value={job.status} /><span className="ml-2">{job.reason}</span></div><span className="text-xs text-muted-foreground">{formatDate(job.created_at)}</span></div>)}</div></section>}
    {can(permissions, "leaderboards.recalculate") && <section><h3 className="font-heading text-lg font-bold">Аварийная замена оценки</h3><p className="mt-1 text-sm text-muted-foreground">Доступна только во время паузы или при подведении итогов. После замены рейтинг обновится автоматически.</p><div className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_1fr_auto]"><select value={datasetVersionId} onChange={(event) => setDatasetVersionId(event.target.value)} className="h-10 border border-border bg-background px-3 text-sm"><option value="">Выберите версию датасета</option>{datasetOptions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><select value={metricVersionId} onChange={(event) => setMetricVersionId(event.target.value)} className="h-10 border border-border bg-background px-3 text-sm"><option value="">Оставить текущую метрику</option>{metricOptions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Причина замены" className="rounded-none" /><Button variant="outline" onClick={() => mutation.mutate({ type: "scoring" })} disabled={!datasetVersionId || reason.trim().length < 3}><Settings2 size={15} /> Заменить</Button></div></section>}
...
function CompetitionsSection({ permissions, requestAction }) {
  ...
  return <>
    ...
    <TableShell loading={query.isLoading} error={query.error} empty={!rows.length}><table className="w-full min-w-[1080px] text-left text-sm"><thead className="border-b border-border bg-secondary/70 text-xs uppercase text-muted-foreground"><tr><th className="px-5 py-3">Соревнование</th><th className="px-4 py-3">Организатор</th><th className="px-4 py-3">Статус</th><th className="px-4 py-3">Дедлайн</th><th className="px-4 py-3">Участники</th><th className="px-5 py-3 text-right">Действия</th></tr></thead><tbody className="divide-y divide-border">{rows.map((competition) => <tr key={competition.id} className="hover:bg-secondary/35"><td className="px-5 py-4"><p className="font-semibold">{displayName(competition)}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{competition.slug || competition.id}</p></td><td className="px-4 py-4">{competition.organization_name || competition.company_name || "ML-Арена"}</td><td className="px-4 py-4"><Status value={competition.status} /></td><td className="px-4 py-4 text-muted-foreground">{formatDate(competition.submission_deadline || competition.deadline)}</td><td className="px-4 py-4 font-semibold">{competition.participants_count ?? 0}</td><td className="px-5 py-4"><ActionMenu>
      ...
      {competition.origin === "community" && ["submitted_for_review", "moderation"].includes(competition.status) && can(permissions, "competitions.publish") && <ActionButton tone="primary" onClick={() => requestAction({ title: "Одобрить соревнование сообщества", description: "Проверьте датасет, скрытые ответы и настройки доступа перед одобрением.", confirm: "Одобрить", reason: true, run: (reason) => api.admin.moderateCommunityCompetition(competition.id, { decision: "approve", reason }), invalidate: ["admin", "competitions"] })}><CheckCircle2 size={13} /> Одобрить community</ActionButton>}
      ...
      {["draft", "moderation", "scheduled"].includes(competition.status) && can(permissions, "competitions.publish") && <ActionButton tone="primary" onClick={() => execute(competition, "publish", "Опубликовать соревнование", "Проверьте датасет, метрику, правила, настройки доступа и дедлайн.")}><Play size={13} /> Опубликовать</ActionButton>}
      ...
      {competition.status === "paused" && can(permissions, "competitions.pause") && <ActionButton tone="primary" onClick={() => execute(competition, "resume", "Возобновить соревнование", "Проверьте дедлайн и готовность организатора.")}><Play size={13} /> Возобновить</ActionButton>}
...
function CustomMetricModeration({ permissions, requestAction }) {
  const [selected, setSelected] = useState(null);
  const query = useQuery({ queryKey: ["admin", "metric-reviews"], queryFn: api.admin.pendingMetricReviews });
  const rows = listRows(query.data);
  const version = selected?.version;
  const runs = useQuery({ queryKey: ["admin", "metric-validation-runs", version?.id], queryFn: () => api.admin.metricValidationRuns(version.id), enabled: Boolean(version?.id) });
  const openSource = async () => {
    const source = await api.admin.metricVersionSource(version.id);
    const url = source.url || source.download_url || source.signed_url;
    if (url) window.open(url, "_blank", "noopener,noreferrer");
  };
  const action = (entry, type, title, permission, danger = false) => {
    if (!can(permissions, permission)) return;
    requestAction({ title, description: `${entry.metric?.name || entry.metric?.code || "Custom-метрика"} · версия ${entry.version?.version || "—"}. Комментарий сохранится в истории.`, confirm: title, danger, reason: true, run: (comment) => api.admin.metricVersionAction(entry.version.id, type, comment), invalidate: ["admin", "metric-reviews"] });
  };
  if (!can(permissions, "metrics.read")) return null;
  return <section className="mb-8"><SectionHeading title="Custom-метрики" description="Кандидатные версии организаций: sandbox-тесты, исходный код и решение модератора." count={listTotal(query.data)} /><TableShell loading={query.isLoading} error={query.error} empty={!rows.length}><div className="divide-y divide-border">{rows.map((entry) => <article key={entry.version?.id || entry.id} className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><Status value={entry.version?.moderation_status} /><span className="border border-border px-2 py-1 text-[10px] font-semibold">{entry.version?.input_contract}</span><span className="text-xs text-muted-foreground">v{entry.version?.version}</span></div><h3 className="mt-3 font-semibold">{entry.metric?.name || entry.metric?.code}</h3><p className="mt-1 font-mono text-xs text-muted-foreground">{entry.metric?.code} · {entry.metric?.owner_organization_id || "организация"}</p></div><ActionMenu><ActionButton onClick={() => setSelected(entry)}><History size={13} /> Проверки</ActionButton><ActionButton onClick={async () => { setSelected(entry); const source = await api.admin.metricVersionSource(entry.version.id); const url = source.url || source.download_url || source.signed_url; if (url) window.open(url, "_blank", "noopener,noreferrer"); }}><FileText size={13} /> Исходник</ActionButton>{can(permissions, "metrics.review") && <ActionButton onClick={() => requestAction({ title: "Повторить sandbox-тест", description: entry.metric?.name || entry.metric?.code, confirm: "Запустить тест", reason: false, run: () => api.admin.testMetricVersion(entry.version.id), invalidate: ["admin", "metric-reviews"] })}><RefreshCw size={13} /> Тест</ActionButton>}{entry.version?.moderation_status === "pending_review" && can(permissions, "metrics.review") && <><ActionButton tone="primary" onClick={() => action(entry, "approve", "Одобрить метрику", "metrics.review")}><CheckCircle2 size={13} /> Одобрить</ActionButton><ActionButton onClick={() => action(entry, "request-changes", "Запросить изменения", "metrics.review")}><CircleAlert size={13} /> На доработку</ActionButton><ActionButton tone="danger" onClick={() => action(entry, "reject", "Отклонить метрику", "metrics.review", true)}><X size={13} /> Отклонить</ActionButton></>}</ActionMenu></article>)}</div></TableShell>{selected && <Dialog.Root open onOpenChange={(open) => !open && setSelected(null)}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] max-h-[86vh] w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto border border-border bg-card p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><Dialog.Title className="font-heading text-xl font-bold">История проверок</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">{selected.metric?.name} · версия {version?.version}</Dialog.Description></div><button type="button" onClick={() => setSelected(null)} className="flex h-9 w-9 items-center justify-center border border-border"><X size={16} /></button></div><div className="mt-5 flex gap-2"><Button variant="outline" size="sm" onClick={openSource}><FileText size={14} /> Открыть исходник</Button></div><div className="mt-5 divide-y divide-border border border-border">{runs.isLoading ? <LoadingRows /> : listRows(runs.data).length ? listRows(runs.data).map((run) => <div key={run.id} className="p-4"><div className="flex items-center justify-between gap-3"><Status value={run.status} /><span className="text-xs text-muted-foreground">{formatDate(run.created_at)}</span></div>{run.error_message && <p className="mt-2 text-sm text-destructive">{run.error_message}</p>}{run.details && <pre className="mt-3 max-h-48 overflow-auto bg-secondary/50 p-3 text-xs">{JSON.stringify(run.details, null, 2)}</pre>}</div>) : <p className="p-5 text-sm text-muted-foreground">Запуски проверки пока не появились.</p>}</div></Dialog.Content></Dialog.Portal></Dialog.Root>}</section>;
}

function ModerationSection({ permissions, requestAction }) {
  const [status, setStatus] = useState("new");
  const query = useQuery({ queryKey: ["admin", "moderation", status], queryFn: () => api.admin.moderationReports({ status, limit: 50, offset: 0 }), enabled: can(permissions, "moderation.read"), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
...
function ContentSectionLegacy({ permissions, requestAction }) {
  ...
  const transition = (post, action, title, permission, options = {}) => requestAction({ title, description: `${post.title}. Проверьте обязательные поля перед изменением статуса.`, confirm: title, danger: options.danger, reason: false, run: () => api.admin.blogPostAction(post.id, action, options.body), invalidate: ["admin", "blog"], disabled: !can(permissions, permission) });
  return (
    <>
...
function resourceDefaults(type, item, mode) {
  ...
  if (type === "metrics") return {
    ...
    allowed_task_types: (version.allowed_task_types || []).join(", "), publish: false,
  };
  ...
  if (type === "tasks") return {
    ...
    target_column: version.target_column || "target", prediction_column: version.prediction_column || "prediction",
    group_column: version.group_column || "",
    purpose: version.purpose || "competition",
    practice_enabled: version.practice_enabled ?? true,
    challenge_enabled: version.challenge_enabled ?? false,
    challenge_difficulty: version.challenge_difficulty || "easy",
    benchmark_score: version.benchmark_score ?? "", benchmark_rating: version.benchmark_rating ?? "",
    benchmark_version: version.benchmark_version || "", challenge_calibrated: version.challenge_calibration_status === "calibrated",
  };
  ...
  if (type === "plans") return {
    code: item?.code || "", name: item?.name || "", description: item?.description || "",
    billing_period: item?.billing_period || "month", amount: item?.amount == null ? 0 : Number(item.amount) / 100,
    compare_at_amount: item?.compare_at_amount == null ? "" : Number(item.compare_at_amount) / 100,
...
function resourcePayload(type, form, mode) {
  ...
  if (type === "tasks") {
    ...
    const version = {
      ...
      task_type: form.task_type, purpose: form.purpose, difficulty: form.difficulty || null, metric_version_id: form.metric_version_id,
      ...
      target_column: form.target_column.trim(), prediction_column: form.prediction_column.trim(),
      group_column: form.group_column.trim() || null,
    };
  ...
  if (type === "plans") {
    const body = {
      ...
      amount: Math.round(Number(form.amount) * 100), compare_at_amount: form.compare_at_amount === "" ? null : Math.round(Number(form.compare_at_amount) * 100),
...
function AdminSelect({ value, onChange, options, disabled = false }) {
  ...
  return <div className="space-y-7">
    ...
    <section>
      ...
      <div className="mt-4 grid gap-5 border border-border bg-secondary/25 p-5 sm:grid-cols-2">
        ...
        <div className="flex items-end"><p className="min-h-10 w-full border-l-2 border-primary bg-card px-4 py-2 text-xs leading-5 text-muted-foreground">Укажите обычную сумму, например <strong className="text-foreground">6 900 ₽</strong>.</p></div>
      </div>
    </section>
    ...
    <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-end justify-between gap-4"><div><h3 className="font-heading text-lg font-bold">Возможности и лимиты</h3><p className="mt-1 text-xs text-muted-foreground">Добавьте код возможности и укажите её значение: число, текст, да или нет.</p></div>{entitlements && <Button type="button" variant="outline" size="sm" onClick={() => setEntitlements([...entitlementEntries, [`option_${entitlementEntries.length + 1}`, true]])}><Plus size={14} /> Добавить</Button>}</div>
...
function ResourceFields({ type, mode, form, update, organizationOptions = [], metricOptions = [], datasetOptions = [], datasetValidation, onValidateDataset, validatingDataset = false }) {
  ...
  if (type === "metrics") return <div className="grid gap-5 sm:grid-cols-2">
    ...
    {mode !== "edit" && <><AdminField label="Направление"><AdminSelect value={form.direction} onChange={(value) => update("direction", value)} options={[["maximize", "Максимизировать"], ["minimize", "Минимизировать"]]} /></AdminField><AdminField label="Формат"><Input value={form.display_format} onChange={(event) => update("display_format", event.target.value)} className={input} /></AdminField><AdminField label="Тип реализации"><Input value="Встроенная метрика платформы" disabled className={input} /></AdminField><AdminField label="Ключ реализации"><Input value={form.implementation_key} onChange={(event) => update("implementation_key", event.target.value)} placeholder="f1, roc_auc, rmse" className={input} /></AdminField><AdminField label="Типы задач через запятую" wide><Input value={form.allowed_task_types} onChange={(event) => update("allowed_task_types", event.target.value)} placeholder="classification, nlp" className={input} /></AdminField></>}
  ...
  if (type === "tasks") {
    ...
    const compatibleDatasets = datasetOptions.filter((item) => !item.usage_scope || item.usage_scope === form.purpose || item.id === form.dataset_version_id);
...
function AdminCheckbox({ checked, onChange, label, wide = false }) {
  return <label className={cn("flex cursor-pointer items-center gap-3 text-sm font-semibold", wide && "sm:col-span-2")}><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-4 w-4 accent-primary" />{label}</label>;
}

function taskReleasePayload(form) {
  const challenge = form.challenge_enabled;
  return {
    practice_enabled: form.practice_enabled,
    challenge_enabled: challenge,
    challenge_difficulty: challenge ? form.challenge_difficulty : null,
    benchmark_score: challenge ? String(form.benchmark_score) : null,
    benchmark_rating: challenge ? Number(form.benchmark_rating) : null,
    benchmark_version: challenge ? form.benchmark_version.trim() : null,
    challenge_calibration_status: challenge && form.challenge_calibrated ? "calibrated" : null,
  };
}
...
function ResourceEditorDialog({ editor, onClose, onSaved }) {
  ...
  const organizationReferencesEnabled = ["tasks", "datasets"].includes(editor?.type) && editor?.mode === "create";
  ...
  const datasetValidationQuery = useQuery({ queryKey: ["admin", "task-dataset-validation", form?.dataset_version_id], queryFn: () => api.admin.datasetValidation(form.dataset_version_id), enabled: Boolean(taskReferencesEnabled && form?.purpose === "duel" && form?.dataset_version_id), retry: false });
  ...
  const datasetOptions = listRows(datasetsQuery.data).filter((item) => item.status !== "archived" && item.current_version?.id).map((item) => ({ id: item.current_version.id, label: `${item.name} · версия ${item.current_version.version}${item.current_version.usage_scope ? ` · ${item.current_version.usage_scope}` : " · не закреплён"}`, files: item.current_version.files || [], usage_scope: item.current_version.usage_scope || null }));
  useEffect(() => {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editor.type === "plans" && form.currency.trim().length !== 3) {
        throw new Error("Укажите трёхбуквенный код валюты, например RUB.");
      }
      ...
      if (editor.type === "tasks" && editor.mode !== "edit" && !form.dataset_version_id) {
        throw new Error("Выберите версию датасета. Если список пуст, сначала создайте датасет и его версию.");
      }
      if (editor.type === "tasks" && editor.mode !== "edit" && !["competition", "duel"].includes(form.purpose)) {
        throw new Error("Выберите назначение версии: соревнование или дуэль.");
      }
      if (editor.type === "tasks" && editor.mode !== "edit" && (!form.target_column.trim() || !form.prediction_column.trim())) {
        throw new Error("Укажите целевую колонку и колонку прогноза.");
      }
      if (editor.type === "tasks" && editor.mode !== "edit" && form.task_type === "ranking" && !form.group_column.trim()) {
        throw new Error("Для задачи ранжирования укажите колонку группировки.");
      }
      const selectedTaskDataset = datasetOptions.find((item) => item.id === form.dataset_version_id);
      if (editor.type === "tasks" && editor.mode !== "edit" && selectedTaskDataset?.usage_scope && selectedTaskDataset.usage_scope !== form.purpose) {
        throw new Error(`Датасет уже закреплён с usage_scope=${selectedTaskDataset.usage_scope}. Выберите совместимую версию.`);
      }
      if (editor.type === "tasks" && editor.mode !== "edit" && form.purpose === "duel" && datasetValidationQuery.data?.status !== "valid") {
        throw new Error("Перед выпуском в дуэли запустите проверку датасета и убедитесь, что она завершилась успешно.");
      }
      if (editor.type === "tasks" && editor.mode !== "edit" && form.purpose === "duel" && form.challenge_enabled
        && (!String(form.benchmark_score).trim() || !String(form.benchmark_rating).trim() || !form.benchmark_version.trim() || !form.challenge_calibrated)) {
        throw new Error("Для вызова ML-Арены заполните benchmark, его рейтинг и версию, затем подтвердите калибровку.");
      }
      ...
      if (editor.type === "tasks" && editor.mode !== "edit" && form.purpose === "duel") {
        const taskId = result?.id || editor.item?.id;
        const versionId = result?.current_version?.id || result?.current_version_id;
        try {
          await api.admin.releaseTaskVersion(taskId, versionId, taskReleasePayload(form));
        } catch (releaseError) {
          onSaved(result);
          toast({ title: "Задача сохранена как черновик", description: `Не удалось открыть задачу для дуэлей: ${apiErrorMessage(releaseError)}`, variant: "destructive" });
          return;
        }
      }
      onSaved(result);
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !pending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className={cn("fixed left-1/2 top-1/2 z-[101] flex max-h-[92vh] w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none", ["plans", "tasks"].includes(editor.type) ? "max-w-5xl" : "max-w-3xl")}><div className="flex items-start justify-between gap-5 border-b border-border p-5 md:px-6"><div><Dialog.Title className="font-heading text-2xl font-extrabold">{title}</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Заполните обязательные поля и сохраните изменения.</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-6"><ResourceFields type={editor.type} mode={editor.mode} form={form} update={update} organizationOptions={organizationOptions} metricOptions={metricOptions} datasetOptions={datasetOptions} datasetValidation={datasetValidationQuery.data} onValidateDataset={() => validateTaskDataset.mutate()} validatingDataset={validateTaskDataset.isPending} />{(organizationsQuery.error || metricsQuery.error || datasetsQuery.error) && <div className="mt-5"><ErrorState compact error={organizationsQuery.error || metricsQuery.error || datasetsQuery.error} /></div>}{error && <div className="mt-5 border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}</div><div className="flex justify-end gap-2 border-t border-border bg-card/95 p-4"><Button variant="outline" onClick={onClose} disabled={pending}>Отмена</Button><Button onClick={save} disabled={pending || referencesLoading}>{pending ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Сохранить</Button></div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
...
function TaskReleaseDialog({ task, tasks, onClose, onSaved }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: onSaved,
    onError: (releaseError) => setError(apiErrorMessage(releaseError)),
  });
  useEffect(() => {
    setForm(resourceDefaults("tasks", task, "release"));
    setError("");
  }, [task]);
  if (!task || !version) return null;
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const challengeCoverage = ["easy", "medium", "advanced"].map((difficulty) => ({
    difficulty,
    ready: tasks.some((item) => item.current_version?.task_type === version.task_type
      && item.current_version?.purpose === "duel"
      && item.current_version?.exposure_status === "released"
      && item.current_version?.challenge_enabled
      && item.current_version?.challenge_difficulty === difficulty),
  }));
  const submit = () => {
    setError("");
    if (version.purpose !== "duel") {
      setError("Release доступен только для версии с purpose=duel. Сначала измените назначение private-версии.");
      return;
    }
...
function DatasetFilesDialog({ datasetId, permissions, onClose, onSaved }) {
  ...
  const attach = async () => {
    ...
    setError("");
    try {
      const upload = await uploadFile(file, "task_dataset");
      const requiredVisibility = kind === "participant_bundle" ? "participant" : ["public_labels", "private_labels"].includes(kind) ? "evaluator_only" : visibility;
      await api.admin.attachDatasetFile(datasetId, version.id, { upload_id: upload.id, kind, visibility: requiredVisibility, position: files.length });
      setFile(null);
      await query.refetch();
      onSaved();
    } catch (uploadError) {
  ...
  const removeFile = async (entry) => {
    if (!version?.id || !window.confirm(`Удалить файл «${entry.file?.original_filename || entry.kind}»?`)) return;
    setDeletingFileId(entry.id);
    setError("");
    try {
      await api.admin.deleteDatasetFile(datasetId, version.id, entry.id);
      await query.refetch();
      if (showValidation) await validation.refetch();
      onSaved();
    } catch (deleteError) {
      setError(deleteError.status === 409
        ? "Эта версия уже используется. Создайте новую версию датасета и измените файлы в ней."
        : deleteError.message);
    } finally {
  ...
  const lockedVisibility = kind === "participant_bundle" ? "participant" : ["public_labels", "private_labels"].includes(kind) ? "evaluator_only" : null;
return <Dialog.Root open onOpenChange={(open) => !open && !pending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[90vh] w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between gap-4 border-b border-border p-5"><div><Dialog.Title className="font-heading text-2xl font-extrabold">Файлы датасета</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Текущая версия {version?.version ?? "—"} · использование: {version?.usage_scope || "ещё не закреплено"}</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5"><div className="divide-y divide-border border border-border">{files.length ? files.map((entry) => <div key={entry.id} className="grid gap-x-4 gap-y-2 px-4 py-3 text-sm sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center lg:grid-cols-[minmax(150px,.8fr)_minmax(0,1.2fr)_120px_auto]"><span className="min-w-0 break-words font-semibold">{entry.kind}</span><span className="min-w-0 break-all text-muted-foreground">{entry.file?.original_filename || entry.id}</span><span className="text-muted-foreground sm:col-start-1 lg:col-start-auto">{entry.visibility}</span><div className="flex flex-wrap gap-2 sm:col-start-2 sm:row-span-2 sm:row-start-1 sm:justify-end lg:col-start-auto lg:row-span-1 lg:row-start-auto">{(entry.visibility !== "evaluator_only" || can(permissions, "datasets.answers.read")) && <Button size="sm" variant="outline" onClick={() => downloadFile(entry)} disabled={Boolean(downloadingFileId)}>{downloadingFileId === entry.id && <Loader2 size={14} className="animate-spin" />} Скачать</Button>}{can(permissions, "datasets.write") && <Button size="sm" variant="outline" className="text-destructive hover:text-destructive" onClick={() => removeFile(entry)} disabled={Boolean(deletingFileId)} aria-label={`Удалить ${entry.file?.original_filename || entry.kind}`}>{deletingFileId === entry.id ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />} Удалить</Button>}</div></div>) : <p className="p-5 text-sm text-muted-foreground">Файлы ещё не прикреплены.</p>}</div>{version && <div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" onClick={() => setShowValidation(true)}><Search size={15} /> Последняя проверка</Button>{can(permissions, "datasets.validate") && <Button variant="outline" onClick={() => validate.mutate()} disabled={validate.isPending}>{validate.isPending ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />} Проверить сейчас</Button>}</div>}{showValidation && <div className="mt-4 border border-border bg-background p-4">{validation.isLoading ? <p className="text-sm text-muted-foreground">Загружаем результат…</p> : validation.error ? <p className="text-sm text-muted-foreground">Проверка ещё не запускалась.</p> : <><div className="flex items-center justify-between"><p className="font-semibold">Результат проверки</p><Status value={validation.data?.status} /></div><div className="mt-3 space-y-2">{(validation.data?.checks || []).map((check) => <div key={check.code} className="flex gap-2 text-sm">{check.passed ? <CheckCircle2 size={16} className="shrink-0 text-emerald-500" /> : <CircleAlert size={16} className="shrink-0 text-destructive" />}<span>{check.message}</span></div>)}</div></>}</div>}{version ? <div className="mt-6 grid gap-4 sm:grid-cols-2"><AdminField label="Назначение"><AdminSelect value={kind} onChange={(value) => { setKind(value); if (value === "participant_bundle") setVisibility("participant"); else if (["public_labels", "private_labels"].includes(value)) setVisibility("evaluator_only"); }} options={[["participant_bundle", "Данные участника (ZIP)"], ["public_labels", "Публичные ответы (CSV)"], ["private_labels", "Приватные ответы (CSV)"], ["baseline", "Бейзлайн"], ["statement_attachment", "Материал условия"]]} /></AdminField><AdminField label="Видимость"><AdminSelect value={lockedVisibility || visibility} onChange={setVisibility} disabled={Boolean(lockedVisibility)} options={[["participant", "Участникам"], ["public", "Публично"], ["organizer", "Организатору"], ["admin", "Администраторам"], ["evaluator_only", "Только проверяющей системе"]]} /></AdminField><AdminField label="Файл" wide><Input type="file" accept={kind === "participant_bundle" ? ".zip,application/zip" : ["public_labels", "private_labels"].includes(kind) ? ".csv,text/csv,application/csv" : kind === "baseline" ? ".py,.ipynb,text/x-python,application/x-ipynb+json" : ".csv,.json,.pdf,.zip"} onChange={(event) => setFile(event.target.files?.[0] || null)} className="h-11 rounded-none" /></AdminField>{error && <div className="border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive sm:col-span-2">{error}</div>}<div className="flex justify-end sm:col-span-2"><Button onClick={attach} disabled={!file || pending}>{pending ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />} Загрузить и прикрепить</Button></div></div> : <p className="mt-6 border border-amber-500/25 bg-amber-500/5 p-4 text-sm">Сначала создайте версию датасета.</p>}</div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}

function AdminCustomMetricDialog({ onClose, onSaved }) {
  const { toast } = useToast();
  const organizationsQuery = useQuery({ queryKey: ["admin", "custom-metric-organizations"], queryFn: () => api.admin.organizations({ limit: 100, offset: 0 }) });
  const organizations = listRows(organizationsQuery.data).filter((item) => item.status === "active");
  const [form, setForm] = useState({ code: "", name: "", description: "", owner_organization_id: "", direction: "maximize", display_format: "0.0000", input_contract: "tabular_v1", visibility: "public", allowed_task_types: ["classification"] });
  const [file, setFile] = useState(null);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState("");
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const toggleTaskType = (taskType) => update("allowed_task_types", form.allowed_task_types.includes(taskType) ? form.allowed_task_types.filter((item) => item !== taskType) : [...form.allowed_task_types, taskType]);
  const submit = async () => {
    setError("");
    if (!form.code.trim() || !form.name.trim() || !file) return setError("Укажите код, название и файл metric.py.");
    if (!/^[a-z][a-z0-9_]*$/.test(form.code.trim())) return setError("Код должен начинаться с буквы и содержать только a-z, цифры и подчёркивание.");
    if (!file.name.toLowerCase().endsWith(".py") || file.size > 128 * 1024) return setError("Нужен файл .py размером не больше 128 КБ.");
    setPending(true);
    try {
      const ownerOrganizationId = form.owner_organization_id || null;
      const upload = await uploadFile(file, "metric_source", ownerOrganizationId ? { organization_id: ownerOrganizationId } : {});
      const metric = await api.admin.createCustomMetric({
        code: form.code.trim(), name: form.name.trim(), description: form.description.trim() || null,
        owner_organization_id: ownerOrganizationId, direction: form.direction, display_format: form.display_format.trim() || "0.0000",
        source_upload_id: upload.id, input_contract: form.input_contract, allowed_task_types: form.allowed_task_types,
        visibility: ownerOrganizationId ? form.visibility : "public",
      });
      const versionId = metric.current_version?.id || metric.current_version_id;
      if (versionId) {
        try { await api.admin.testMetricVersion(versionId); } catch { /* The endpoint may start validation during creation. */ }
      }
      toast({ title: "Custom-метрика создана", description: versionId ? "Проверка реализации запущена." : "Метрика сохранена в черновиках." });
      onSaved();
    } catch (submitError) {
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !pending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[92vh] w-[calc(100%-2rem)] max-w-4xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between gap-4 border-b border-border p-5 md:px-7"><div><Dialog.Title className="font-heading text-2xl font-extrabold">Новая custom-метрика</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Загрузите Python-реализацию и назначьте владельца. Перед использованием метрика пройдёт sandbox-проверку.</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-7"><div className="grid gap-5 sm:grid-cols-2"><AdminField label="Код"><Input value={form.code} onChange={(event) => update("code", event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ""))} placeholder="business_cost" className="rounded-none font-mono" /></AdminField><AdminField label="Название"><Input value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Business cost" className="rounded-none" /></AdminField><AdminField label="Описание" wide><Textarea maxLength={5000} value={form.description} onChange={(event) => update("description", event.target.value)} className="min-h-24 rounded-none" /></AdminField><AdminField label="Владелец"><AdminSelect value={form.owner_organization_id} onChange={(value) => { update("owner_organization_id", value); if (!value) update("visibility", "public"); }} options={[["", "Платформа ML-Арена"], ...organizations.map((item) => [item.id, item.name || item.slug])]} /></AdminField><AdminField label="Видимость"><AdminSelect value={form.owner_organization_id ? form.visibility : "public"} onChange={(value) => update("visibility", value)} disabled={!form.owner_organization_id} options={[["owner_only", "Только владельцу"], ["public", "Публичная"]]} /></AdminField><AdminField label="Цель"><AdminSelect value={form.direction} onChange={(value) => update("direction", value)} options={[["maximize", "Максимизировать"], ["minimize", "Минимизировать"]]} /></AdminField><AdminField label="Входные данные"><AdminSelect value={form.input_contract} onChange={(value) => update("input_contract", value)} options={[["tabular_v1", "Табличные данные"], ["ranking_v1", "Ранжирование"]]} /></AdminField><AdminField label="Формат результата"><Input value={form.display_format} onChange={(event) => update("display_format", event.target.value)} placeholder="0.0000" className="rounded-none" /></AdminField><AdminField label="Файл metric.py"><Input type="file" accept=".py,text/x-python,text/plain" onChange={(event) => setFile(event.target.files?.[0] || null)} className="h-11 rounded-none" /></AdminField><div className="sm:col-span-2"><p className="mb-3 text-sm font-semibold">Совместимые типы задач</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{TASK_TYPE_OPTIONS.map(([value, label]) => <button key={value} type="button" onClick={() => toggleTaskType(value)} className={cn("min-h-11 border px-3 py-2 text-left text-xs font-semibold", form.allowed_task_types.includes(value) ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-muted-foreground hover:border-primary/40")}>{label}</button>)}</div></div>{error && <div className="border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive sm:col-span-2">{error}</div>}</div></div><div className="flex justify-end gap-2 border-t border-border p-4 md:px-7"><Button variant="outline" onClick={onClose} disabled={pending}>Отмена</Button><Button onClick={submit} disabled={pending || organizationsQuery.isLoading}>{pending ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />} Создать и проверить</Button></div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
...
const RESOURCE_TYPES = {
  ...
  metrics: { label: "Метрики", singular: "метрику", icon: Gauge, query: api.admin.metrics, permission: "metrics.read", write: "metrics.write" },
...
function ResourcesSection({ permissions, requestAction, initialResource = "datasets" }) {
  const available = Object.entries(RESOURCE_TYPES).filter(([, resource]) => can(permissions, resource.permission));
  const [active, setActive] = useState(available.some(([id]) => id === initialResource) ? initialResource : available[0]?.[0] || "datasets");
  const [editor, setEditor] = useState(null);
  ...
  const actions = (item) => {
    ...
    return <ActionMenu>
      ...
      {active === "datasets" && item.current_version?.id && can(permissions, "datasets.validate") && <ActionButton tone="primary" onClick={() => run(item, "Проверить датасет", "Будут проверены структура и доступность файлов текущей версии.", () => api.admin.validateDatasetVersion(item.current_version.id))}><CheckCircle2 size={13} /> Проверить</ActionButton>}
      ...
      {active === "metrics" && item.current_version?.implementation_type === "custom" && item.current_version?.moderation_status === "approved" && can(permissions, "metrics.disable") && <ActionButton tone="danger" onClick={() => run(item, "Отключить версию метрики", "Новые расчёты с этой версией будут запрещены.", () => api.admin.metricVersionAction(item.current_version.id, "disable", "Отключена администратором"), { danger: true })}><Ban size={13} /> Отключить версию</ActionButton>}
      {active === "tasks" && item.current_version?.id && item.current_version?.exposure_status === "private" && !item.current_version?.purpose_locked_at && item.status !== "archived" && <ActionButton onClick={() => { const nextPurpose = item.current_version.purpose === "duel" ? "competition" : "duel"; run(item, "Изменить назначение версии", `Назначение будет изменено на ${nextPurpose}. После первого использования изменить его будет нельзя.`, () => api.admin.updateTaskVersionPurpose(item.id, item.current_version.id, nextPurpose)); }}><RefreshCw size={13} /> Сменить на {item.current_version.purpose === "duel" ? "соревнование" : "дуэли"}</ActionButton>}
      {active === "tasks" && item.current_version?.id && item.current_version?.purpose === "duel" && item.current_version?.exposure_status === "private" && item.status !== "archived" && <ActionButton tone="primary" onClick={() => setTaskRelease(item)}><ArrowUpRight size={13} /> Выпустить для дуэлей</ActionButton>}
      ...
      {active === "datasets" && item.status === "archived" && <ActionButton tone="primary" onClick={() => run(item, "Восстановить датасет", displayName(item), () => api.admin.restoreDataset(item.id))}><RotateCcw size={13} /> Восстановить</ActionButton>}
  ...
  const createAction = !canCreate ? null : active === "metrics"
    ? <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setEditor({ type: active, mode: "create", item: null })}><Plus size={16} /> Встроенная</Button><Button onClick={() => setCustomMetricOpen(true)}><Upload size={16} /> Custom-метрика</Button></div>
    : <Button onClick={() => setEditor({ type: active, mode: "create", item: null })}><Plus size={16} /> Создать {current.singular}</Button>;
  return <><SectionHeading title="Ресурсы платформы" description="Создание, версии, публикация и архивирование сущностей, от которых зависит работа платформы." count={listTotal(query.data)} action={createAction} /><div className="mb-4 flex flex-wrap gap-2">{available.map(([id, resource]) => { const Icon = resource.icon; return <button key={id} type="button" onClick={() => setActive(id)} className={`inline-flex h-10 items-center gap-2 border px-3 text-sm font-semibold ${active === id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:text-primary"}`}><Icon size={15} />{resource.label}</button>; })}</div><TableShell loading={query.isLoading} error={query.error} empty={!rows.length}><div className="divide-y divide-border">{rows.map((item) => <div key={item.id} className="grid gap-4 px-5 py-4 xl:grid-cols-[minmax(260px,1fr)_160px_150px_minmax(280px,auto)] xl:items-center"><div><p className="font-semibold">{displayName(item)}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{item.code || item.slug || item.id}</p>{item.current_version && <div className="mt-2 flex flex-wrap gap-1.5"><span className="border border-border bg-secondary px-2 py-1 text-[10px] font-semibold">v{item.current_version.version}</span>{active === "tasks" ? <>{item.current_version.purpose === "competition" && <span className="border border-amber-500/25 bg-amber-500/5 px-2 py-1 text-[10px] font-semibold text-amber-700">Для соревнований</span>}{item.current_version.purpose === "duel" && <span className="border border-primary/25 bg-primary/5 px-2 py-1 text-[10px] font-semibold text-primary">Для дуэлей</span>}{item.current_version.purpose_locked_at && <span className="border border-border px-2 py-1 text-[10px] font-semibold text-muted-foreground">Назначение закреплено</span>}{item.current_version.exposure_status === "competition_only" && <span className="border border-primary/25 bg-primary/5 px-2 py-1 text-[10px] font-semibold text-primary">Закреплена в соревновании</span>}{item.current_version.practice_enabled && <span className="border border-border px-2 py-1 text-[10px] font-semibold">Практика</span>}{item.current_version.challenge_enabled && <span className="border border-emerald-500/25 bg-emerald-500/5 px-2 py-1 text-[10px] font-semibold text-emerald-700">ML-Арена · {{ easy: "лёгкий", medium: "средний", advanced: "продвинутый" }[item.current_version.challenge_difficulty] || item.current_version.challenge_difficulty}</span>}</> : <span className="text-[10px] text-muted-foreground">{item.current_version.usage_scope ? `usage: ${item.current_version.usage_scope}` : item.current_version.exposure_status || item.current_version.direction || "текущая"}</span>}</div>}</div><Status value={item.status} /><p className="text-sm text-muted-foreground">{formatDate(item.updated_at || item.created_at)}</p><div className="xl:justify-self-end">{actions(item)}</div></div>)}</div></TableShell><ResourceEditorDialog editor={editor} onClose={() => setEditor(null)} onSaved={saved} /><TaskReleaseDialog task={taskRelease} tasks={rows} onClose={() => setTaskRelease(null)} onSaved={() => { setTaskRelease(null); invalidate(); toast({ title: "Доступ к задаче обновлён" }); }} /><DatasetFilesDialog datasetId={datasetFilesId} permissions={permissions} onClose={() => setDatasetFilesId(null)} onSaved={invalidate} />{customMetricOpen && <AdminCustomMetricDialog onClose={() => setCustomMetricOpen(false)} onSaved={() => { setCustomMetricOpen(false); invalidate(); queryClient.invalidateQueries({ queryKey: ["admin", "metric-reviews"] }); }} />}</>;
}
...
export default function Admin() {
  ...
  const [activeSection, setActiveSection] = useState(requestedSection || "dashboard");
    ...
    <div className="admin-page min-h-full bg-background">
      ...
      <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[250px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 p-3 py-5 sm:p-6 lg:p-8 xl:p-10">
          ...
          {active === "resources" && <ResourcesSection {...shared} initialResource={searchParams.get("resource") || "datasets"} />}
