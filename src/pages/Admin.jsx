...
function AdminCustomMetricDialog({ onClose, onSaved }) {
  const { toast } = useToast();
  const [form, setForm] = useState({ code: "", name: "", description: "", direction: "maximize", display_format: "0.0000", input_contract: "tabular_v1", allowed_task_types: ["classification"] });
  const [file, setFile] = useState(null);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState("");
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  ...
  const submit = async () => {
    ...
    if (!form.allowed_task_types.length) return setError("Выберите хотя бы один тип задачи.");
    setPending(true);
    try {
      const upload = await uploadFile(file, "metric_source");
      const metric = await api.admin.createCustomMetric({
        ...
        direction: form.direction, display_format: form.display_format.trim() || "0.0000",
  ...
  return <Dialog.Root open onOpenChange={(open) => !open && !pending && onClose()}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-[101] flex max-h-[92vh] w-[calc(100%-2rem)] max-w-4xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden border border-border bg-card shadow-2xl focus:outline-none"><div className="flex items-start justify-between gap-4 border-b border-border p-5 md:px-7"><div><Dialog.Title className="font-heading text-2xl font-extrabold">Новая custom-метрика</Dialog.Title><Dialog.Description className="mt-1 text-sm text-muted-foreground">Загрузите Python-реализацию метрики. После создания она пройдёт проверку.</Dialog.Description></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center border border-border"><X size={17} /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-7"><div className="grid gap-5 sm:grid-cols-2"><AdminField label="Код"><Input value={form.code} onChange={(event) => update("code", event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ""))} placeholder="business_cost" className="rounded-none font-mono" /></AdminField><AdminField label="Название"><Input value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Business cost" className="rounded-none" /></AdminField><AdminField label="Описание" wide><Textarea maxLength={5000} value={form.description} onChange={(event) => update("description", event.target.value)} className="min-h-24 rounded-none" /></AdminField><AdminField label="Цель"><AdminSelect value={form.direction} onChange={(value) => update("direction", value)} options={[["maximize", "Максимизировать"], ["minimize", "Минимизировать"]]} /></AdminField><AdminField label="Входные данные"><AdminSelect value={form.input_contract} onChange={(value) => update("input_contract", value)} options={[["tabular_v1", "Табличные данные"], ["ranking_v1", "Ранжирование"]]} /></AdminField><AdminField label="Формат отображения результата"><Input value={form.display_format} onChange={(event) => update("display_format", event.target.value)} placeholder="0.0000" className="rounded-none" /><p className="mt-1 text-xs text-muted-foreground">0.0000 означает четыре знака после запятой в показанном результате. На расчёт метрики это не влияет.</p></AdminField><AdminField label="Файл metric.py"><Input type="file" accept=".py,text/x-python,text/plain" onChange={(event) => setFile(event.target.files?.[0] || null)} className="h-11 rounded-none" /></AdminField><div className="sm:col-span-2"><p className="mb-3 text-sm font-semibold">Совместимые типы задач</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{TASK_TYPE_OPTIONS.map(([value, label]) => <button key={value} type="button" onClick={() => toggleTaskType(value)} className={cn("min-h-11 border px-3 py-2 text-left text-xs font-semibold", form.allowed_task_types.includes(value) ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-muted-foreground hover:border-primary/40")}>{label}</button>)}</div></div>{error && <div className="border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive sm:col-span-2">{error}</div>}</div></div><div className="flex justify-end gap-2 border-t border-border p-4 md:px-7"><Button variant="outline" onClick={onClose} disabled={pending}>Отмена</Button><Button onClick={submit} disabled={pending}>{pending ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />} Создать и проверить</Button></div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
