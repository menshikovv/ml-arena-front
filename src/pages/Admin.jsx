...
function ResourceFields({ type, mode, form, update, organizationOptions = [], metricOptions = [], datasetOptions = [], datasetValidation, onValidateDataset, validatingDataset = false }) {
  ...
  if (type === "tasks") {
    ...
    {mode !== "edit" && <section className="order-1"><h3 className="font-heading text-xl font-bold">Сначала выберите назначение</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">Назначение передаётся backend как purpose и блокируется после первого использования версии.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{[["competition", "Для соревнования", "Останется private. При выборе в соревновании backend закрепит версию и датасет за соревнованиями."], ["duel", "Для дуэлей", "После создания будет released. Backend закрепит версию и датасет за дуэлями."]].map(([value, label, text]) => <button key={value} type="button" onClick={() => { update("purpose", value); if (selectedDataset?.usage_scope && selectedDataset.usage_scope !== value) update("dataset_version_id", ""); }} className={cn("min-h-28 border p-5 text-left transition-colors", form.purpose === value ? "border-primary bg-primary/5 shadow-[inset_3px_0_0_hsl(var(--primary))]" : "border-border bg-background hover:border-primary/30")}><span className="flex items-center gap-2 text-base font-bold">{value === "competition" ? <Trophy size={18} /> : <Swords size={18} />}{label}</span><span className="mt-2 block text-xs leading-5 text-muted-foreground">{text}</span></button>)}</div>
    ...
    try {
      ...
      if (editor.type === "tasks" && editor.mode !== "edit" && form.purpose === "duel") {
        ...
        try {
          ...
          onSaved(result);
          toast({ title: "Задача сохранена как private", description: `Выпуск отклонён сервером: ${apiErrorMessage(releaseError)}`, variant: "destructive" });
          return;
        }
      }
      onSaved(result);
