  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 md:py-8">
      ...
      <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <StaggerItem><StatCard icon={Users} label="Пользователи" value={profiles?.length || 0} color="#7C3AED" /></StaggerItem>
        <StaggerItem><StatCard icon={Trophy} label="Соревнования" value={competitions?.length || 0} color="#06B6D4" /></StaggerItem>
        <StaggerItem><StatCard icon={Building2} label="Дуэли" value={duels?.length || 0} color="#EC4899" /></StaggerItem>
        <StaggerItem><StatCard icon={Database} label="Сущностей" value={8} color="#F59E0B" /></StaggerItem>
      </Stagger>

      {/* Actions */}
      <Stagger className="grid sm:grid-cols-2 gap-3 mb-6" delay={0.12}>
        <StaggerItem>
      ...
      {competitions?.length === 0 ? (
        ...
        <Reveal delay={0.2}>
          <Card className="overflow-hidden bg-card/40 border-border">
            <Stagger className="divide-y divide-border" delay={0.22}>
            {competitions?.map((c) => (
              <StaggerItem key={c.id} className="flex items-center gap-3 px-4 py-3">
      ...
      <Reveal delay={0.28}>
        <Card className="overflow-hidden bg-card/40 border-border">
          <Stagger className="divide-y divide-border" delay={0.3}>
          {duels?.length === 0 && <p className="text-sm text-muted-foreground text-center py-6">Дуэлей нет</p>}
          {duels?.map((d) => (
            <StaggerItem key={d.id} className="flex items-center gap-3 px-4 py-3">
