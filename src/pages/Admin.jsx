...
function UsersSection({ permissions, requestAction }) {
  ...
  const query = useQuery({ queryKey: ["admin", "users", deferredSearch, status, role], queryFn: () => api.admin.users({ q: deferredSearch, status, role, limit: 100, offset: 0 }), placeholderData: keepPreviousData });
...
function OrganizationsSection({ permissions, requestAction }) {
  const query = useQuery({ queryKey: ["admin", "organizations"], queryFn: () => api.admin.organizations({ limit: 100, offset: 0 }), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
...
function CompetitionsSection({ permissions, requestAction }) {
  const [status, setStatus] = useState("all");
  const query = useQuery({ queryKey: ["admin", "competitions", status], queryFn: () => api.admin.competitions({ status, limit: 100, offset: 0 }), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
...
function SubmissionsSection({ permissions, requestAction }) {
  const [status, setStatus] = useState("all");
  const query = useQuery({ queryKey: ["admin", "submissions", status], queryFn: () => api.admin.submissions({ status, limit: 100, offset: 0 }), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
...
function ModerationSection({ permissions, requestAction }) {
  ...
  const query = useQuery({ queryKey: ["admin", "moderation", status], queryFn: () => api.admin.moderationReports({ status, limit: 100, offset: 0 }), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
...
function ContentSection({ permissions, requestAction }) {
  ...
  const posts = useQuery({ queryKey: ["admin", "blog", deferredSearch, status], queryFn: () => api.admin.blogPosts({ q: deferredSearch, status, limit: 100, offset: 0, sort: "-updated_at" }), placeholderData: keepPreviousData });
...
function ResourcesSection({ permissions }) {
  ...
  const query = useQuery({ queryKey: ["admin", "resources", active], queryFn: () => current.query({ limit: 100, offset: 0 }), enabled: Boolean(current), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
...
function AuditSection() {
  ...
  const query = useQuery({ queryKey: ["admin", "audit", deferredAction], queryFn: () => api.admin.auditLogs({ action: deferredAction, limit: 100, offset: 0 }), placeholderData: keepPreviousData });
  const rows = listRows(query.data);
