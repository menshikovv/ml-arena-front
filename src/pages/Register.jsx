...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await register({ email: normalizedEmail, nickname: form.nickname.trim(), marketingConsent: form.marketing, attribution });
      setRegisteredEmail(normalizedEmail);
      setForm((current) => ({ ...current, password: "", confirmation: "" }));
    } catch (submitError) {
      setError(submitError.message === "email_exists"
        ? "Аккаунт с таким email уже существует. Войдите или восстановите пароль."
        : submitError.message === "nickname_exists"
    ...
    return (
      <AuthLayout wide icon={CheckCircle2} title="Предрегистрация создана" subtitle={`Мы подготовили письмо для ${maskEmail(registeredEmail)}. Подтвердите email, чтобы завершить регистрацию.`}>
        <div className="space-y-5">
          ...
          <Button asChild className="w-full"><Link to="/verify-email?token=demo">Подтвердить email</Link></Button>
          <FounderSeasonTelegramCard compact embedded />
        </div>
      </AuthLayout>
    );
  }
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={form.password} onChange={(event) => update("password", event.target.value)} className="h-11 px-10" maxLength={64} required />
            ...
            {form.password && !passwordValid && <p className="text-xs text-destructive">8–64 символа, минимум одна буква и одна цифра.</p>}
          </div>
          <div className="space-y-2">
        ...
        <Button type="submit" className="h-12 w-full" disabled={!isValid || loading}>
          {loading ? <><Loader2 className="animate-spin" size={16} /> Создаём аккаунт…</> : "Создать аккаунт и пройти предрегистрацию"}
        </Button>
      </form>
    </AuthLayout>
  );
